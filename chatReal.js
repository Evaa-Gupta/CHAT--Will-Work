...
firebase.databse().ref(roomName).push({
  Name: UserName,
  Message: message,
  like: 0
})
  ...
  firebase.database().ref("/"+roomName).on('value', function(snapshot) { 
    ...
    snapshot.forEach(function(childSnapshot) { 
      ...
      if(childKey != "purpose") {
         ...
         messagename= "<h4>"+name+"<img src='tick.png' class='tick'></h4>"
         ...
         thumbsUp = "<span class= 'glyphicon glyphicon-thumbs-up'> </span> </button> &nbsp"
         messageDelete= "<button class='btn btn-danger' id= "+firebase_message_id+" onclick= 'updateDelete(this.id)'>"
         trash = "<span class= 'glyphicon glyphicon-trash'>Delete</span> </button> <hr> " 
